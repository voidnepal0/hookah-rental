    ...
    const fetchOrders = async () => {
      try {
        ...
        const params: FetchOrdersParams = {
          ...
          orderType:
            orderTypeFilter !== "all"
              ? (orderTypeFilter as "dine_in" | "takeaway" | "delivery")
              : undefined,
          sortBy,
          sortOrder,
        };
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 py-8">
        ...
        {/* Filters and Search */}
        <div
          className={`p-6 rounded-lg mb-6 ${isDark ? "bg-gray-900" : "bg-gray-50"}`}
        >
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div className="lg:col-span-2">
              <div className="relative">
                <Search
                  size={20}
                  className={`absolute left-3 top-1/2 transform -translate-y-1/2 ${isDark ? "text-gray-400" : "text-gray-600"}`}
                />
                <input
                  type="text"
        ...
        {/* Orders Table */}
        <div
          className={`rounded-lg overflow-hidden ${isDark ? "bg-gray-900" : "bg-white border border-gray-200"}`}
        >
          {loading ? (
            <div className="p-8">
              ...
              <table className="w-full">
                ...
                >
                  {orders.map((order) => {
                    ...
                    const productsTotalAmount = order.productRequests.reduce(
                      (acc, curr) => {
                        let currentProductTotal =
                          (curr.soldAtPrice || 0) * (curr.quantity || 0);
                        let currentAddonsTotal = curr.addons.reduce(
                          (acc, curr) => {
                            return (
                              acc + curr.quantity * curr.addon.additionalPrice
                            );
                          },
                          0,
                        );
                        return acc + currentProductTotal + currentAddonsTotal;
                      },
                      0,
                    );
                    return (
                      ...
                      >
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div>
                            ...
                            >
                              {order.productRequests.length} item
                              {order.productRequests.length !== 1 ? "s" : ""}
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
