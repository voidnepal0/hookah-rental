...
import { getOrderById, settleOrder, updateOrderStatus } from '@/services/api/orderApi';
...
const OrderDetailsPage = () => {
  ...
  const handleUpdateStatus = async (status: 'pending' | 'completed' | 'cancelled' | 'preparing' | 'ready') => {
    if (!orderId) return;
    ...
    <div className={`min-h-screen ${isDark ? 'bg-black text-white' : 'bg-white text-black'}`}>
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className={`p-6 rounded-lg ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div>
                  <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Order Number</p>
                  <p className="font-semibold">#{order.orderNumber}</p>
                </div>
                <div>
                  <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Order Type</p>
            ...
            <div className={`p-6 rounded-lg ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {order.quickDeliveryAddress && (
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Delivery Address</p>
                      <p className="font-medium">{order.quickDeliveryAddress}</p>
                    </div>
                  </div>
                )}
              </div>
              {order.notes && (
                <div className="mt-4">
                  <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-1`}>Notes</p>
            ...
            <div className={`p-6 rounded-lg ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
              ...
              <div className="space-y-4">
                {order.productRequests.map((item) => (
                  <div key={item.id} className="flex items-center gap-4 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                    ...
                    <div className="text-right">
                      <div className="font-semibold">Rs. {item.product.sellingPrice * item.quantity}</div>
                      <div className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                        Rs. {item.product.sellingPrice} each
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className={`p-6 rounded-lg ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
                ...
                <div className="space-y-4">
                  {order.serviceRequests.map((service) => (
                    <div key={service.id} className="flex items-center justify-between p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                      <div>
                      ...
                      <div className="text-right">
                        <div className="font-semibold">Rs. {service.price * service.quantity}</div>
                        <div className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
          ...
          <div className="lg:col-span-1">
            <div className={`p-6 rounded-lg sticky top-8 ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className={`flex justify-between text-xl font-bold pt-3 border-t ${isDark ? 'border-gray-700' : 'border-gray-300'}`}>
                  <span>Total:</span>
                  <span>Rs. {order.totalAmount}</span>
                </div>
              </div>

              <div className={`p-3 rounded-lg text-sm ${isDark ? 'bg-gray-800' : 'bg-gray-200'}`}>
      ...
      {showPaymentForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className={`w-full max-w-md mx-4 p-6 rounded-lg ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
            <h3 className="text-xl font-semibold mb-4">Settle Order Payment</h3>
            
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-2">
                  Total Amount: <span className="font-bold">Rs. {order?.totalAmount}</span>
                </label>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setPaymentDetails(prev => ({
                    ...prev,
                    paymentMethod: e.target.value as SettleOrderRequest['paymentMethod']
                  }))}
                  className={`w-full px-4 py-2 rounded-lg border ${
                    isDark 
                      ? 'bg-black border-gray-700 text-white' 
                ...
                <div className={`p-3 rounded-lg ${isDark ? 'bg-gray-800' : 'bg-gray-100'}`}>
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold">
                      Rs. {Math.max(0, paymentDetails.receivedAmount - (order?.totalAmount || 0))}
                    </span>
                  </div>
                </div>
              )}
